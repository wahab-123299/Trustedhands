  ...
  try {
    ...
    if (slots && Array.isArray(slots)) {
      for (const slot of slots) {
        if (!slot.startTime || !slot.endTime) throw new AppError('VALIDATION_ERROR', 'Each slot must have startTime and endTime');
        const start = parseTimeCtrl(slot.startTime);
        const end = parseTimeCtrl(slot.endTime);
        if (end - start < 30) throw new AppError('VALIDATION_ERROR', 'Each slot must be at least 30 minutes');
        if (start >= end) throw new AppError('VALIDATION_ERROR', 'End time must be after start time');
      }
      for (let i = 0; i < slots.length; i++) {
        for (let j = i + 1; j < slots.length; j++) {
          const a = slots[i], b = slots[j];
          if (parseTimeCtrl(a.startTime) < parseTimeCtrl(b.endTime) && parseTimeCtrl(a.endTime) > parseTimeCtrl(b.startTime)) {
...
availabilityController.createRecurringPattern = async (req, res, next) => {
  try {
    ...
    for (const slot of timeSlots) {
      if (parseTimeCtrl(slot.endTime) - parseTimeCtrl(slot.startTime) < 30) throw new AppError('VALIDATION_ERROR', 'Each slot must be at least 30 minutes');
    }
...
availabilityController.checkAvailability = async (req, res, next) => {
  try {
    ...
    if (!availability || availability.isBlocked) {
      return res.json({ success: true, data: { isAvailable: false, reason: availability?.isBlocked ? 'Date is blocked' : 'No availability set' } });
    }
...
availabilityController.cancelBooking = async (artisanId, date, jobId) => {
  try {
    ...
    const slot = availability.slots.find(s => s.jobId?.toString() === jobId.toString());
    if (slot) {
      slot.isBooked = false;
      slot.jobId = null;
      availability.updatedAt = new Date();
