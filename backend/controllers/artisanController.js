...
export const registerArtisan = asyncHandler(async (req, res) => {
  const { name, email, password, phone, profession, experience, bio, address } = req.body;
...
export const authArtisan = asyncHandler(async (req, res) => {
  ...
  if (artisan && (await artisan.matchPassword(password))) {
    res.json({
      _id: artisan._id,
      name: artisan.name,
      email: artisan.email,
...
export const searchArtisans = asyncHandler(async (req, res) => {
  ...
  const query = {};
  if (skill) query.profession = { $regex: skill, $options: "i" };
