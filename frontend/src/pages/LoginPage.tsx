...
const LoginPage = () => {
  ...
  const [formData, setFormData] = useState<{ email: string; password: string; rememberMe: boolean }>({
    email: '',
    password: '',
    rememberMe: false,
  });
  ...
  const validateForm = () => {
    const newErrors: Record<'email' | 'password', string> = {} as Record<'email' | 'password', string>;
